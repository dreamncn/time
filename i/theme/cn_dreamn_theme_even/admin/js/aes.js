...
var  layer,$,notice,form;
layui.use(['jquery','layer','notice','form'],function() {
    layer = layui.layer;
        $ = layui.jquery;
        notice=layui.notice;
        form=layui.form;
...
var postParma = function (data, key) {
    //获得加密字符串
    return {"param": Encrypt(JSON.stringify(data), key)};
};
 ...
 * @return {string}
 */
var Encrypt = function (word, keys) {
    var key = CryptoJS.enc.Utf8.parse(keys);
    var srcs = CryptoJS.enc.Utf8.parse(word);
    var encrypted = CryptoJS.AES.encrypt(srcs, key, {mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.Pkcs7});
    return encrypted.toString();
};
var saveChange = function (url, data,msg=null,callback=null) {
    if(msg===null)msg="正在处理，请稍后...";
    notice.msg(msg, {icon: 4, close: false});
    $.get(key, function (d) {
        const key = d.key;
        $.ajax({
            url: url,
            type: 'POST',
            data: postParma(data, key),
            dataType: "json",
            ...
            if(result.length===2){
                ...
                switch(type){
                    case 'select':{
                        ...
                        $('[lay-filter='+select+'] [lay-filter='+name+']').val(val[Obj]);
                        $('[lay-filter='+select+'] [lay-filter='+name+'] option').removeAttr('selected');
                        $('[lay-filter='+select+'] [lay-filter='+name+'] option[value='+val[Obj]+']').prop('selected', true);

                        break;
                    }
                    case 'radio':{
                        $('[lay-filter='+select+'] [lay-filter='+name+']').removeAttr('checked');
                        $('[lay-filter='+select+'] [lay-filter='+name+'][value='+val[Obj]+']').prop('checked', true);
                        break;
                    }
                    case 'check':{
                        if(val[Obj]){
                            $('[lay-filter='+select+'] [lay-filter='+name+']').prop('checked', true);
                        }else{
                            $('[lay-filter='+select+'] [lay-filter='+name+']').removeAttr('checked');
                        }
                        break;
                ...
                $('[lay-filter='+select+'] [lay-filter='+result[0]+']').val(val[Obj]);
            }
        }
